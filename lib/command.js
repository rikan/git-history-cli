...
function showHistory(url, options) {
    ...
    if (options.text) {
        _options.text = '--grep="' + options.text + '"';
    } else {
        _options.text = '';
    }
    if (options.pretty) {
    ...
    return exec(constants.COMMAND_GIT_LOG + `${_options.pretty} ${_options.branch} ${_options.text} ${_options.number} ${_options.until} ${_options.since}`)
}
